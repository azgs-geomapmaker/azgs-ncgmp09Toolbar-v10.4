    ...
    static class domainUpdater
    {
        ...
        private static void UpdateCodedValueDomain(IWorkspace theWorkspace, string DomainName, string SourceClassName, string CodeFieldName, string ValueFieldName)
        {
            ...
            try
            {
                ...
                List<string> uniqDom = new List<string>();

                while (theRow != null)
                {
                    if (uniqDom.Contains(theRow.get_Value(codeFld).ToString()))
                    {
                        MessageBox.Show(DomainName + " has a duplicate value: " + theRow.get_Value(codeFld).ToString() + " and not added to the domain. ");
                    }
                    else
                    {
                        theDomain.AddCode(theRow.get_Value(codeFld), theRow.get_Value(valueFld).ToString());
                        uniqDom.Add(theRow.get_Value(codeFld).ToString());
                    }
            ...
            {
                MessageBox.Show(DomainName + " was not updated.\nError Message: " +
                                e.StackTrace + "\nThis is may be because an exclusive schema lock could not be obtained.", "NCGMP Tools");
            }
            finally
            {
